...
class SiameseBERT_v4():

    def __init__(self, trainable=False, name="SiameseBERT_v4"):
    ...
    def build_encoder_part(self, verbose=False):
        ...
        bert_encoder = self.BERTencoder(trainable=False)
        ...
        conc = layers.Concatenate()([x4, x3, x2['pooled_output'], x1['pooled_output']]) # concatenate the difference and cosine distance
        x = layers.Dropout(0.1)(conc)

        x = layers.Dense(512, activation='relu')(x)
        x = layers.Dropout(0.2)(x)

        x = layers.Dense(256, activation='tanh', name="conc_layer")(x)
